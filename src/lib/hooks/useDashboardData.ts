  ...
  const fetchStrategyComparison = useCallback(async () => {
    try {
      ...
      if (ids.length <= 1) {
        ...
        setErrorStrategyComparison(new Error('Niewystarczająca liczba strategii do porównania'));
        return;
      }
      ...
      const bestMetrics: Record<string, string> = {
        ...
        fcp: comparisonData.results.find(
          r => r.id === comparisonData.comparison.metrics_comparison.fcp?.best
        )?.strategy_type || '',
        tti: comparisonData.results.find(
          r => r.id === comparisonData.comparison.metrics_comparison.tti?.best
        )?.strategy_type || '',
        lcp: comparisonData.results.find(
          r => r.id === comparisonData.comparison.metrics_comparison.lcp?.best
        )?.strategy_type || '',
        fid: comparisonData.results.find(
          r => r.id === comparisonData.comparison.metrics_comparison.fid?.best
        )?.strategy_type || '',
        ttfb: comparisonData.results.find(
          r => r.id === comparisonData.comparison.metrics_comparison.ttfb?.best
        )?.strategy_type || ''
      };
